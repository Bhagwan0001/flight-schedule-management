export const filterFlights = (flights, filters, search) => {
    const searchText = search.trim().toLowerCase();
    return flights.filter((f) => {
        const matchesDate = !filters.fromDate ||
            !filters.toDate ||
            (f.startDate <= filters.toDate &&
                f.endDate >= filters.fromDate);
        const matchesDays = !filters.days?.length ||
            f.daysOfOperation.some((d) => filters.days?.includes(d));
        const matchesStatus = !filters.status || f.status === filters.status;
        const matchesAOC = !filters.aoc || f.aoc === filters.aoc;
        const matchesBody = !filters.bodyType || f.bodyType === filters.bodyType;
